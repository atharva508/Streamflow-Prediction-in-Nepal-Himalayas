...
# Load datasets and transfer learning datasets (if applicable)
print("Loading datasets...")
datasets = load_and_process_data(config['data_paths'])
...
# Identify all feature names (excluding 'streamflow') across all datasets
print("Identifying all feature names...")
all_features = set()
for _, dataset in combined_datasets:
...
# Compute global min and max for each column across all datasets
...
def data_generator(datasets, n_steps_in, n_steps_out, min_max_dict, features, split='train'):
    global test_data_dict  # Use a global dictionary to store the test data and possibly validation data
...
# Generate training and validation data by iterating through the generators
# Note: Since generators can only be iterated once, we'll create tf.data.Dataset from generator functions
